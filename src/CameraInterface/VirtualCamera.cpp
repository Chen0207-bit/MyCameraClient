...
VirtualCamera::VirtualCamera(const CameraMetaInfo& info)
    ...
    , m_frameIndex(0)
{
    m_cameraParams.push_back({"AcquisitionControl", "ExposureTime", "12000 us",
                              "Controls how long the virtual sensor accumulates light for each frame.", {}});
    m_cameraParams.push_back({"AcquisitionControl", "FrameRate", "24 fps",
                              "The target number of frames produced every second while grabbing is active.", {}});
    m_cameraParams.push_back({"AnalogControl", "Gain", "6 dB",
                              "Amplifies the image signal. Higher gain brightens the image but can increase noise.", {}});
    m_cameraParams.push_back({"TriggerControl", "TriggerMode", "Off",
                              "When Off, the camera grabs continuously. When On, it waits for an external trigger.",
                              {"Off", "On"}});
}
...
uint32_t VirtualCamera::writeParam(const CameraParam& param)
{
    for (auto& currentParam : m_cameraParams)
    {
        if (currentParam.Name == param.Name)
        {
            currentParam.Value = param.Value;
            currentParam.Options = param.Options;
            currentParam.Tips = param.Tips;
            currentParam.Group = param.Group;
            return CHONGMING_OK;
        }
    }
